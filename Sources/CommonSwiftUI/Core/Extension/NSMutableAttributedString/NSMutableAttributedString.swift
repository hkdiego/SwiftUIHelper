...
extension NSMutableAttributedString {
    ...
    /**
    Returns a new mutable string with new color.


    - Parameters:
    - color: new color
    - textToFind: part of String to be painted or ignore it to paint full
    - returns: A new string new color.
    */
    public func paint(with color: UIColor, _ textToFind: String? = nil) -> NSMutableAttributedString {
        let foundRange = self.mutableString.range(of: textToFind ?? self.string)
...
extension NSMutableAttributedString {
    ...
    public func setAsLink(textToFind: String, linkURL: String, color: UIColor, font: UIFont? = nil) -> Self {
        ...
        if foundRange.location != NSNotFound {
            ...
            addAttribute(NSAttributedString.Key.foregroundColor, value: color, range: foundRange)
            if let font = font { addAttribute(NSAttributedString.Key.font, value: font, range: foundRange) }
        }
        return self
    }
}
